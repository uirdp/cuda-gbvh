...
static void debug_dirty_keys_bits_distribution(const std::vector<ulonglong2>& keys)
{
    ...
    printf("=== dirty keys bits distribution ===\n");
    for (auto& kv : hist) {
        printf("bits=%d : keys=%d\n", kv.first, kv.second);
    }
}

static void debug_frame_leaves_stats(DeviceScene& h_scene)
{
    int num_leaves = h_scene.num_frame_leaves;

    std::vector<GPU_LeafNode> h_leaves(num_leaves);
    ...
    for (int i = 0; i < num_leaves; i++) {
        total_tris += h_leaves[i].tri_count;

        if (h_leaves[i].tri_count == 0)
            empty_leaves++;

        max_tris = std::max(max_tris, h_leaves[i].tri_count);
    }

    printf("=== frame leaves stats ===\n");
    printf("num_leaves   = %d\n", num_leaves);
    printf("total_tris   = %lld\n", total_tris);
    printf("empty_leaves = %d\n", empty_leaves);
    printf("max_tris     = %d\n", max_tris);
    printf("=========================\n");
}

void debug_validate_curr_bvh(DeviceScene& h_scene)
{
    int num_nodes = 0;
    CHECK_CUDA(cudaMemcpy(&num_nodes,
    ...
    std::vector<GPU_BVH_Node> nodes(num_nodes);
    CHECK_CUDA(cudaMemcpy(nodes.data(),
                          h_scene.curr_bvh_nodes,
                          sizeof(GPU_BVH_Node) * num_nodes,
                          cudaMemcpyDeviceToHost));

    printf("validate BVH: root=%d nodes=%d leaves=%d\n",
           h_scene.curr_bvh_root_node_idx,
           num_nodes,
           num_leaves);

    if (h_scene.curr_bvh_root_node_idx < 0 ||
        h_scene.curr_bvh_root_node_idx >= num_nodes) {
        printf("ERROR: root out of range\n");
    }

    for (int i = 0; i < num_nodes; ++i) {
        const auto& n = nodes[i];

        auto check_child = [&](int idx, int type, const char* side) {
            if (type == GPU_CHILD_LEAF) {
                if (idx < 0 || idx >= num_leaves) {
                    printf("ERROR node %d %s leaf idx out of range: %d / %d\n",
                           i, side, idx, num_leaves);
                }
            } else if (type == GPU_CHILD_NODE) {
                if (idx < 0 || idx >= num_nodes) {
                    printf("ERROR node %d %s node idx out of range: %d / %d\n",
                           i, side, idx, num_nodes);
                }
            } else {
                printf("ERROR node %d %s bad child type=%d idx=%d\n",
                       i, side, type, idx);
            }
        };

        check_child(n.left_idx,  n.left_type,  "left");
        check_child(n.right_idx, n.right_type, "right");
    }
}
...
int main(int argc, char **argv)
{
...
#else
    if (nthreads)
        fprintf(stderr, "Warning: Single-thread only. The -n option will be ignored.\n");
#endif

    /* ファイルの読み込み */
    printf("Parameter file = \"%s\"\n", param_file.c_str());
    if (RT_ReadParamFile(param_file, param, scene))
        exit(1);
    printf("Obj file = \"%s\"\n", param.get_obj_file_name().c_str());
    if (RT_ReadObjFile(param, scene))
        exit(1);
    ...
    vector<DirtyKey> h_dirty_keys = build_sorted_dirty_keys_from_set(dirty_keys);
    ...
    for (int frame = 0; frame < num_frames; frame++)
    {
        if (scene.scenario.size() > 2)
        {
            ...
            if (frame > 0)
            {
                ...
                double update_ms = std::chrono::duration<double, std::milli>(end_update - start_update).count();
                ...
                printf("BVH updated on GPU.\n");
                CHECK_CUDA(cudaGetLastError());
                CHECK_CUDA(cudaDeviceSynchronize());
                int h_num_curr = 0;
                CHECK_CUDA(cudaMemcpy(&h_num_curr,
                                      h_device_scene.num_curr_bvh_nodes,
                                      sizeof(int),
                                      cudaMemcpyDeviceToHost));
                ...
                h_device_scene.num_curr_bvh_nodes_value = h_num_curr;
            ...
            promote_curr_to_prev(d_scene, h_device_scene);
...
#ifdef _WIN32
    output_prefix = "build-win2\\results\\f";
#elif __linux__

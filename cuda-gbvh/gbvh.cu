...
static inline void add_dirty_grid_key_raw(
...
static inline void add_dirty_grid_ancestor_keys_from_leaf(
    DirtyKeySet& dirty_keys,
    const LeafNode* leaf)
{
    if (!leaf)
    ...
    // leaf 自身の grid key も GRID dirty として入れる
    add_dirty_grid_key_raw(dirty_keys, code, bits);
...
static inline void add_dirty_leaf_key(
    DirtyKeySet& dirty_keys,
    const LeafNode* leaf)
{
    if (!leaf)
    ...
    // affected internal cluster 判定用
    // CPU GridNode として存在しない中間階層も含めて全部 dirty にする
    add_dirty_grid_ancestor_keys_from_leaf(dirty_keys, leaf);
}
...
static void add_dirty_keys_in_subtree(
    TreeNode* node,
    DirtyKeySet& dirty_keys)
{
    if (!node)
...
static void delete_object(TreeNode *&node, Object *obj, int glevel, vector<LeafNode *> &dirty_leaves, DirtyKeySet &dirty_keys)
{
    ...
    if (node->type == NT_GRID)
    {
        ...
        if (grid->nobjs == 0)
        {
            add_dirty_keys_in_subtree((TreeNode*)grid, dirty_keys);
...
build_dirty_leaf_code_set(const std::vector<DirtyKey> &h_dirty_keys)
{
    std::unordered_set<uint64_t> s;
    s.reserve(h_dirty_keys.size());

    for (const DirtyKey &k : h_dirty_keys)
    {
        if (k.type == DIRTY_KEY_LEAF)
            s.insert(k.leaf_code);
    }

    return s;
}

static bool subtree_contains_dirty_leaf_cpu(
    int root_node_idx,
    const std::vector<GPU_BVH_Node> &prev_nodes,
    const std::vector<GPU_LeafNode> &prev_leaves,
    const std::unordered_set<uint64_t> &dirty_leaf_codes,
    int *out_dirty_leaf_idx = nullptr,
    uint64_t *out_dirty_leaf_code = nullptr)
{
    if (root_node_idx < 0 || root_node_idx >= (int)prev_nodes.size())
        return false;
    ...
    while (!stack.empty())
    {
        ...
        if (node_idx < 0 || node_idx >= (int)prev_nodes.size())
            continue;

        const GPU_BVH_Node &n = prev_nodes[node_idx];

        auto check_leaf = [&](int leaf_idx) -> bool
        {
            if (leaf_idx < 0 || leaf_idx >= (int)prev_leaves.size())
                return false;

            const GPU_LeafNode &leaf = prev_leaves[leaf_idx];

            if (dirty_leaf_codes.find(leaf.leaf_code) != dirty_leaf_codes.end())
            {
                if (out_dirty_leaf_idx)
                    *out_dirty_leaf_idx = leaf_idx;
                if (out_dirty_leaf_code)
                    *out_dirty_leaf_code = leaf.leaf_code;
                return true;
            }
        ...
        if (n.left_type == GPU_CHILD_LEAF)
        {
            if (check_leaf(n.left_idx))
                return true;
        }
        else
        {
            if (n.left_idx >= 0)
                stack.push_back(n.left_idx);
        }

        if (n.right_type == GPU_CHILD_LEAF)
        {
            if (check_leaf(n.right_idx))
                return true;
        }
        else
        {
            if (n.right_idx >= 0)
                stack.push_back(n.right_idx);
        }
    }
...
static bool is_ancestor_grid_key(
    ...
    uint8_t child_bits)
{
    ...
    if ((child_bits - parent_bits) % GRID_BITS_PER_LEVEL != 0)
        return false;

    uint8_t shift = child_bits - parent_bits;

    return parent_code == (child_code >> shift);
}

static void debug_dirty_leaf_ancestors(
    const GPU_LeafNode& leaf,
    const std::unordered_set<GridKey, GridKeyHash>& dirty_grid_keys)
{
    printf("  dirty leaf grid=(%llu,%u) leaf_code=%llu\n",
           (unsigned long long)leaf.grid_code,
           (unsigned)leaf.grid_bits,
           (unsigned long long)leaf.leaf_code);
    ...
    while (bits > 0)
    {
        ...
        bool exists =
            dirty_grid_keys.find(GridKey{code, bits}) != dirty_grid_keys.end();

        printf("    ancestor grid=(%llu,%u) in_dirty_keys=%d\n",
               (unsigned long long)code,
               (unsigned)bits,
               (int)exists);
    }
...
static void debug_check_affected_internal_clusters_cpu(
    ...
    const std::vector<DirtyKey> &h_dirty_keys)
{
    if (!d_affected_clusters || num_affected_clusters <= 0)
        return;

    auto dirty_grid_keys = build_dirty_grid_key_set(h_dirty_keys);
    auto dirty_leaf_codes = build_dirty_leaf_code_set(h_dirty_keys);

    std::vector<GPU_Cluster> h_clusters(num_affected_clusters);

    CHECK_CUDA(cudaMemcpy(
        h_clusters.data(),
        d_affected_clusters,
        sizeof(GPU_Cluster) * num_affected_clusters,
        cudaMemcpyDeviceToHost));

    int checked_internal = 0;
    int bad_internal = 0;

    int bad_node_key_in_dirty = 0;
    int bad_node_is_ancestor = 0;
    int bad_node_not_ancestor = 0;
    int bad_node_ancestor_but_key_missing = 0;

    for (int i = 0; i < num_affected_clusters; ++i)
    {
        const GPU_Cluster &c = h_clusters[i];

        // leaf cluster は対象外
        if (c.node_idx < 0)
            continue;

        // prev internal cluster だけ検査
        if (c.node_source != GPU_NODE_SRC_PREV)
            continue;

        if (c.node_idx < 0 || c.node_idx >= (int)h_prev_nodes.size())
            continue;

        checked_internal++;

        int dirty_leaf_idx = -1;
        uint64_t dirty_leaf_code = 0;

        bool has_dirty = subtree_contains_dirty_leaf_cpu(
            c.node_idx,
            h_prev_nodes,
            h_prev_leaves,
            dirty_leaf_codes,
        ...
        const GPU_BVH_Node &n = h_prev_nodes[c.node_idx];

        bool node_key_in_dirty =
            dirty_grid_keys.find(GridKey{n.grid_code, n.grid_bits}) != dirty_grid_keys.end();

        bool node_is_ancestor = false;

        if (dirty_leaf_idx >= 0 && dirty_leaf_idx < (int)h_prev_leaves.size())
        {
            const GPU_LeafNode &dirty_leaf = h_prev_leaves[dirty_leaf_idx];

            node_is_ancestor = is_ancestor_grid_key(
                n.grid_code,
                n.grid_bits,
                dirty_leaf.grid_code,
                dirty_leaf.grid_bits);
        }

        if (node_key_in_dirty)
            bad_node_key_in_dirty++;

        if (node_is_ancestor)
            bad_node_is_ancestor++;
        else
            bad_node_not_ancestor++;

        if (node_is_ancestor && !node_key_in_dirty)
            bad_node_ancestor_but_key_missing++;
        ...
        if (bad_internal <= 20)
        {
            if (dirty_leaf_idx >= 0 && dirty_leaf_idx < (int)h_prev_leaves.size())
            {
                ...
                printf(
                    "BAD affected internal cluster[%d]: node_idx=%d "
                    "node_grid=(%llu,%u) node_key_in_dirty=%d node_is_ancestor=%d "
                    "dirty_leaf_idx=%d dirty_leaf_grid=(%llu,%u) dirty_leaf_code=%llu\n",
                    i,
                    c.node_idx,
                    (unsigned long long)n.grid_code,
                    (unsigned)n.grid_bits,
                    (int)node_key_in_dirty,
                    (int)node_is_ancestor,
                    dirty_leaf_idx,
                    (unsigned long long)dirty_leaf.grid_code,
                    (unsigned)dirty_leaf.grid_bits,
                    (unsigned long long)dirty_leaf.leaf_code);

                if (bad_internal <= 5)
                {
                    debug_dirty_leaf_ancestors(dirty_leaf, dirty_grid_keys);
                }
            }
            else
            {
                printf(
                    "BAD affected internal cluster[%d]: node_idx=%d "
                    "node_grid=(%llu,%u) dirty_leaf_idx=%d dirty_leaf_code=%llu "
                    "dirty leaf index out of range\n",
                    i,
                    c.node_idx,
                    (unsigned long long)n.grid_code,
                    (unsigned)n.grid_bits,
                    dirty_leaf_idx,
                    (unsigned long long)dirty_leaf_code);
            }
        }
    }

    printf("=== affected internal subtree dirty check ===\n");
    printf("checked internal clusters              = %d\n", checked_internal);
    printf("bad internal clusters                  = %d\n", bad_internal);
    printf("bad node key in dirty_keys             = %d\n", bad_node_key_in_dirty);
    printf("bad node is ancestor of dirty leaf     = %d\n", bad_node_is_ancestor);
    printf("bad node is NOT ancestor of dirty leaf = %d\n", bad_node_not_ancestor);
    printf("ancestor but key missing               = %d\n", bad_node_ancestor_but_key_missing);
    printf("=============================================\n");
}
...
static GPU_Cluster make_cluster_from_prev_leaf_host(
    const std::vector<GPU_LeafNode> &prev_leaves,
    int leaf_idx)
{
    const GPU_LeafNode &leaf = prev_leaves[leaf_idx];
    ...
    c.leaf_idx = leaf_idx; // ここでは prev_complete_leaves 上の index
    c.node_idx = -1;
    c.node_source = GPU_NODE_SRC_NONE;
...
static GPU_Cluster make_cluster_from_prev_node_host(
    const std::vector<GPU_BVH_Node> &prev_nodes,
    int node_idx)
{
    const GPU_BVH_Node &node = prev_nodes[node_idx];
...
static bool expand_subtree_to_clean_leaf_clusters_cpu(
    int root_node_idx,
    const std::vector<GPU_BVH_Node> &prev_nodes,
    const std::vector<GPU_LeafNode> &prev_leaves,
    const std::unordered_set<uint64_t> &dirty_leaf_codes,
    std::vector<GPU_Cluster> &out_clean_leaf_clusters,
    int &dirty_leaf_count,
    int &clean_leaf_count)
{
    if (root_node_idx < 0 || root_node_idx >= (int)prev_nodes.size())
        return false;
    ...
    while (!stack.empty())
    {
        ...
        auto process_leaf = [&](int leaf_idx)
        {
            if (leaf_idx < 0 || leaf_idx >= (int)prev_leaves.size())
                return;
            ...
            out_clean_leaf_clusters.push_back(
                make_cluster_from_prev_leaf_host(prev_leaves, leaf_idx));
            clean_leaf_count++;
        };

        if (node.left_type == GPU_CHILD_LEAF)
        {
            process_leaf(node.left_idx);
        }
...
static void filter_affected_clusters_remove_dirty_cpu(
    GPU_Cluster *&d_affected_clusters,
    int &num_affected_clusters,
    const std::vector<GPU_BVH_Node> &h_prev_nodes,
    const std::vector<GPU_LeafNode> &h_prev_leaves,
    const std::vector<DirtyKey> &h_dirty_keys)
{
    ...
    std::vector<GPU_Cluster> filtered;
    filtered.reserve(h_clusters.size());

    int kept_leaf = 0;
    int skipped_dirty_leaf = 0;

    int kept_clean_internal = 0;
    int expanded_dirty_internal = 0;

    int expanded_clean_leaf = 0;
    int expanded_dirty_leaf = 0;
    ...
    for (int i = 0; i < num_affected_clusters; ++i)
    {
        ...
        // -------------------------------
        // affected leaf cluster
        // -------------------------------
        if (c.leaf_idx >= 0)
        {
            int leaf_idx = c.leaf_idx;

            if (leaf_idx < 0 || leaf_idx >= (int)h_prev_leaves.size())
                continue;

            const GPU_LeafNode &leaf = h_prev_leaves[leaf_idx];
        ...
        // -------------------------------
        if (c.node_idx >= 0 && c.node_source == GPU_NODE_SRC_PREV)
        {
            std::vector<GPU_Cluster> clean_leaf_clusters;
            int local_dirty_leaf_count = 0;
            int local_clean_leaf_count = 0;

            bool has_dirty = expand_subtree_to_clean_leaf_clusters_cpu(
                c.node_idx,
                h_prev_nodes,
                h_prev_leaves,
                dirty_leaf_codes,
                clean_leaf_clusters,
                local_dirty_leaf_count,
                local_clean_leaf_count);

            if (!has_dirty)
            {
                // subtree 全体が clean なので internal cluster のまま残す
                filtered.push_back(c);
                kept_clean_internal++;
            }
            else
            {
                // dirty を含む internal cluster は丸ごと使わない。
                // clean leaf だけに展開して追加する。
                filtered.insert(
                    filtered.end(),
                    clean_leaf_clusters.begin(),
                    clean_leaf_clusters.end());

                expanded_dirty_internal++;
                expanded_clean_leaf += local_clean_leaf_count;
                expanded_dirty_leaf += local_dirty_leaf_count;
            }
    ...
    printf("=== filter affected clusters remove dirty ===\n");
    printf("before affected clusters      = %d\n", num_affected_clusters);
    printf("after affected clusters       = %zu\n", filtered.size());
    printf("kept leaf clusters            = %d\n", kept_leaf);
    printf("skipped dirty leaf clusters   = %d\n", skipped_dirty_leaf);
    printf("kept clean internal clusters  = %d\n", kept_clean_internal);
    printf("expanded dirty internal nodes = %d\n", expanded_dirty_internal);
    printf("expanded clean leaf clusters  = %d\n", expanded_clean_leaf);
    printf("excluded dirty leaves         = %d\n", expanded_dirty_leaf);
    printf("============================================\n");

    CHECK_CUDA(cudaFree(d_affected_clusters));
    d_affected_clusters = nullptr;

    num_affected_clusters = (int)filtered.size();
    ...
    cudaStream_t stream = 0)
{
    ...
    // filter_affected_clusters_remove_dirty_cpu(
    //     d_affected_clusters,
    //     num_affected_clusters,
    //     h_prev_nodes,
    //     h_prev_leaves,
...
{
    DBG_LIFT_CLUSTER = 0,
    DBG_LIFT_CURR_NODE = 1,
    DBG_LIFT_PREV_NODE = 2,
    DBG_LIFT_LEAF_CLUSTER = 3,
    DBG_LIFT_CURR_MISMATCH = 4,
    DBG_LIFT_PREV_MISMATCH = 5,
    DBG_LIFT_COUNT = 6
};

__global__ void kernel_lift_only_target_bits_update_referenced_nodes(
    GPU_Cluster *clusters,
    int n,
    uint8_t target_bits,
    GPU_BVH_Node *curr_nodes,
    GPU_BVH_Node *prev_nodes,
    int *debug_counts)
{
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n)
    ...
    GPU_Cluster &c = clusters[i];

    if (c.grid_bits != target_bits)
        return;

    atomicAdd(&debug_counts[DBG_LIFT_CLUSTER], 1);

    uint64_t old_code = c.grid_code;
    uint8_t old_bits = c.grid_bits;

    uint64_t new_code = old_code;
    uint8_t new_bits = old_bits;

    lift_to_parent(new_code, new_bits);

    c.grid_code = new_code;
    c.grid_bits = new_bits;

    if (c.node_idx >= 0 && c.node_source == GPU_NODE_SRC_CURR)
    {
        atomicAdd(&debug_counts[DBG_LIFT_CURR_NODE], 1);

        GPU_BVH_Node &node = curr_nodes[c.node_idx];

        if (node.grid_code == old_code && node.grid_bits == old_bits)
        {
            node.grid_code = new_code;
            node.grid_bits = new_bits;
        }
        else
        {
            atomicAdd(&debug_counts[DBG_LIFT_CURR_MISMATCH], 1);
        }
    ...
    if (c.node_idx >= 0 && c.node_source == GPU_NODE_SRC_PREV)
    {
        atomicAdd(&debug_counts[DBG_LIFT_PREV_NODE], 1);

        if (prev_nodes == nullptr)
            return;

        GPU_BVH_Node &node = prev_nodes[c.node_idx];
        ...
        {
            atomicAdd(&debug_counts[DBG_LIFT_PREV_MISMATCH], 1);
        }
    ...
    if (c.leaf_idx >= 0)
    {
        atomicAdd(&debug_counts[DBG_LIFT_LEAF_CLUSTER], 1);
    }
}

// __global__ void kernel_lift_only_target_bits_update_nodes(
//     GPU_Cluster *clusters,
//     int n,
//     uint8_t target_bits,
//     GPU_BVH_Node *curr_nodes)
...
//     // この cluster が current BVH node を指しているなら、
//     // node 側の key も同じように更新する。
//     if (c.node_idx >= 0 && c.node_source == GPU_NODE_SRC_CURR)
//     {
//         GPU_BVH_Node &node = curr_nodes[c.node_idx];

//         // 念のため、node key が cluster の旧 key と一致している場合だけ更新する。
//         // 一致していなくても更新したいなら、この if は外してよい。
//         if (node.grid_code == old_code && node.grid_bits == old_bits)
//         {
//             node.grid_code = new_code;
//             node.grid_bits = new_bits;
...
//             kernel_lift_only_target_bits_update_nodes<<<grid, block, 0, stream>>>(
//                 cur,
//                 n,
//                 (uint8_t)target_bits,
//                 d_bvh_nodes);
    ...
    int max_rounds_guard = 256)
{
    ...
    int *d_lift_debug = nullptr;
    CHECK_CUDA(cudaMalloc(&d_lift_debug, sizeof(int) * DBG_LIFT_COUNT));
    CHECK_CUDA(cudaMemsetAsync(d_lift_debug, 0, sizeof(int) * DBG_LIFT_COUNT, stream));
    ...
    while (n > 1 && rounds < max_rounds_guard)
    {
        ...
        // ここが忁E�E�E�E��E�E�E�E�E�E�E��E�E�E�こ�E level めEparent に持ち上げめE
        {
            int block = 256;
            int grid = (n + block - 1) / block;
            kernel_lift_only_target_bits_update_referenced_nodes<<<grid, block, 0, stream>>>(
                cur,
                n,
                (uint8_t)target_bits,
                d_bvh_nodes,
    ...
    int h_lift_debug[DBG_LIFT_COUNT] = {};
    CHECK_CUDA(cudaMemcpy(
        h_lift_debug,
        d_lift_debug,
        sizeof(int) * DBG_LIFT_COUNT,
        cudaMemcpyDeviceToHost));

    printf("=== lift debug ===\n");
    printf("lift clusters       = %d\n", h_lift_debug[DBG_LIFT_CLUSTER]);
    printf("lift curr nodes     = %d\n", h_lift_debug[DBG_LIFT_CURR_NODE]);
    printf("lift prev nodes     = %d\n", h_lift_debug[DBG_LIFT_PREV_NODE]);
    printf("lift leaf clusters  = %d\n", h_lift_debug[DBG_LIFT_LEAF_CLUSTER]);
    printf("curr key mismatch   = %d\n", h_lift_debug[DBG_LIFT_CURR_MISMATCH]);
    printf("prev key mismatch   = %d\n", h_lift_debug[DBG_LIFT_PREV_MISMATCH]);
    printf("==================\n");

    CHECK_CUDA(cudaFree(d_lift_debug));
    ...
    cudaStream_t stream)
{
    ...
    std::vector<GPU_BVH_Node> h_prev_nodes;
    std::vector<GPU_LeafNode> h_prev_leaves;
    ...
    if (has_prev_bvh)
    {
        ...
        // debug 用に prev nodes/leaves を CPU へ戻す
        download_prev_nodes(h_scene, h_prev_nodes);

        h_prev_leaves.resize(h_scene.num_prev_complete_leaves);
        CHECK_CUDA(cudaMemcpy(
            h_prev_leaves.data(),
            h_scene.prev_complete_leaves,
            sizeof(GPU_LeafNode) * h_scene.num_prev_complete_leaves,
        ...
        debug_check_affected_internal_clusters_cpu(
            d_affected_clusters,
            num_affected_clusters,
            h_prev_nodes,
            h_prev_leaves,
    ...
    std::vector<GPU_LeafNode> h_frame(h_scene.num_frame_leaves);
    ...
    // {
    //     printf(
    //         "leaf[%d] leaf_code=%llu grid=(%llu,%u) tris=%d\n",
    //         i,
    //         (unsigned long long)h_frame[i].leaf_code,
    //         (unsigned long long)h_frame[i].grid_code,
    //         h_frame[i].grid_bits,
    //         h_frame[i].tri_count);
    // }
...
struct MaterializeStats
{
    ...
    long long skipped_duplicate_prev_leaf_count = 0;
    long long skipped_duplicate_prev_tri_count = 0;
...
struct LeafGridKey
{
    ...
    bool operator==(const LeafGridKey &other) const
    {
        return code == other.code && bits == other.bits;
    }
};

struct LeafGridKeyHash
{
    size_t operator()(const LeafGridKey &k) const
    {
        size_t h = std::hash<uint64_t>{}(k.code);
        h ^= std::hash<int>{}((int)k.bits) + 0x9e3779b9 + (h << 6) + (h >> 2);
        return h;
    }
};
...
//    // std::unordered_set<uint64_t> &final_leaf_codes, // 追加: 重複コード検出用
                                 ...
                                 int &out_idx, int &out_type, int &out_source)
    {
        ...
        if (child_type == GPU_CHILD_LEAF)
        {
            if (ref.source == GPU_NODE_SRC_PREV)
            {
                ...
                // すでに current/frame 側に同じ leaf_code があるなら、
                // old prev leaf はコピーしない
                if (final_leaf_codes.find(prev_leaf.leaf_code) != final_leaf_codes.end())
                {
                    stats.skipped_duplicate_prev_leaf_count++;
                    stats.skipped_duplicate_prev_tri_count += prev_leaf.tri_count;
                ...
                int new_leaf_idx = (int)h_final_leaves.size();
                h_final_leaves.push_back(prev_leaf);
                final_leaf_codes.insert(prev_leaf.leaf_code);
        ...
        int new_child_idx = materialize_node_recursive(
            ...
            h_final_leaves,
            h_final_nodes,
            final_leaf_codes, // 追加: frame_leaves を渡す
            node_remap,
            stats);
    ...
    DeviceScene &h_scene)
{
    ...
    std::unordered_set<uint64_t> final_leaf_codes;
    final_leaf_codes.reserve(h_final_leaves.size() * 2);

    for (const auto &leaf : h_final_leaves)
    {
        if (leaf.leaf_code != 0)
        {
            final_leaf_codes.insert(leaf.leaf_code);
        }
    }
